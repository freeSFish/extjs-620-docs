            ...
            function render(self) {
                var container  = document.getElementById(self.node),
                    ...
                    click      = function(e) {
                        ...
                        var parent = e.target || e.currentTarget || e.srcElement;
                        parent = ExtL.hasCls(parent, 'isNotLeaf') ? parent : ExtL.up(parent, '.isNotLeaf');
                        parent = parent.parentNode;

                        var data   = parent ? JSON.parse(parent.getAttribute('data-item')) : null,
                            //leaves = parent.parentNode.querySelector('.tree-child-leaves');
                            leaves = parent ? parent.querySelector('.tree-child-leaves') : null;
                ...
                forEach(container.querySelectorAll('.isLeaf'), function(node) {
                    node.onclick = click;
                });
                //forEach(container.querySelectorAll('.tree-expando'), function(node) {
                forEach(container.querySelectorAll('.isNotLeaf'), function(node) {
                    node.onclick = click;
                });
            };
...
(function() {
    var hasClassTemplate = window.isClassTemplate || false,
        ...
        searchHistory = [],
        quickStartTree, guideTree, tree,
        masterSearchList, apiSearchRecords, guideSearchRecords, currentApiPage, currentGuidePage, eventsEl, clicked, pos = {};
     ...
     *
     */
    function onFilterClassCheckboxToggle() {
        filterClassTreeByAccess();
     ...
     *
     */
    function filterClassTreeByAccess() {
    ...
    function onApiTabClick (e) {
        ...
        if (!ExtL.hasCls(target, 'active-tab')) {
            if (myToolkit) {
                ...
                window.location.href = homePath + 'api/' + (product === 'test' ? 'ST.html' : 'Ext.html');
            }
        }
    }
     ...
     *
     */
    function onQuickStartTabClick (e) {
        e = e || window.event;
        var target = e.target || e.srcElement;
     ...
     *
     */
    function filterSearchByToolkit (e) {
        ...
        if (history && history.length) {
            ...
            ExtL.each(history, function (item) {
                ...
                nav.appendChild(ExtL.createElement({
                    ...
                    'data-tip': item.title + ' ' + (isGuide ? myVersion : item.pversion),
                    cn: [{
                        tag: 'span',
                        html: item.title + ' ' + (isGuide ? myVersion : item.pversion) + ' | ',
                        "class": 'history-meta'
                    }, {
                        tag: 'span',
                        html: item.text
    ...
    ExtL.bindReady(function () {
        ...
        if (eventsEl) {
            ExtL.on(eventsEl, 'click', onGuideTabClick);
        }
        eventsEl = ExtL.get('apiTab');
        if (eventsEl) {
            ExtL.on(eventsEl, 'click', onApiTabClick);
        }
        eventsEl = ExtL.get('quickStartTab');
        if (eventsEl) {
            ExtL.on(eventsEl, 'click', onQuickStartTabClick);
        }
        eventsEl = ExtL.get('classicTab');
        if (eventsEl) {
            ExtL.on(eventsEl, 'click', onClassicTabClick);
        }
        eventsEl = ExtL.get('modernTab');
        if (eventsEl) {
            ExtL.on(eventsEl, 'click', onModernTabClick);
        }
   ...
   function toggleExamples (collapse) {
     ...
     *
     */
    function onAccessCheckboxClick() {
        filterByAccess();
    ...
    function initEventHandlers() {
        ...
        if (ExtL.get('modern-search-filter')) {
            ExtL.on(ExtL.get('modern-search-filter'), 'click', filterSearchByToolkit);
        }
        if (ExtL.get('classic-search-filter')) {
            ExtL.on(ExtL.get('classic-search-filter'), 'click', filterSearchByToolkit);
        }
        ...
        if (hasClassTemplate) {
            ...
            ExtL.get('private-class-toggle').onclick = onFilterClassCheckboxToggle;
        ...
        ExtL.each(ExtL.fromNodeList(document.getElementsByClassName('expand-tool')), function (btn) {
            ExtL.up(btn, '.da-inline-fiddle-nav').onclick = onToggleExampleClick;
        });
    ...
    function fetchState (skipSave, returnOnly) {
        ...
        if (isHome || isGuide) {
            ...
            if (myToolkit) {
                toggleNavTab(myToolkit + 'Tab');
            } else {
                toggleNavTab('apiTab');
            }
        } else if (isQuickStart) {
            toggleNavTab('quickStartTab');
        }
        ...
        if (mButton && cButton && state.toolkitFilter) {
            filterSearchByToolkit(state.toolkitFilter);
        }
